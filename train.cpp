...
/********************************************
 Main function
********************************************/
//command_line_argument: a.out arguments_file SVM_Prediction_Threshold  Testing_Category
//arguments file format
/*
* 10 //number of lines in this file
* pos_train_path
* neg_train_path
* pos_test_path
* neg_test_path
* filename_prefix_for_trainDataInput_to_svm_train
* filename_prefix_for_testDataInput_to_svm_predict
* filename_prefix_for_output_of_svm_predict
* method_id
* filename_prefix_for_concise_output_of_svm_predict and also file appending the results.
* God Knows! (the code doesn't have any usage of filepaths[10])
*
*/
...
void trainSVM(char* trainFeatFile, char* trainModelFile)
{
    ...
    else
    {
        ...
        *modelPtr = '_';
        ...
        sprintf(train_cmd, "./liblinear-1.93/train -B 1 %s %s", trainFeatFile, trainModelFile);
...
int main(int argc,char *argv[])
{
	if(argc != 6)
	{
		fprintf(stderr,"Wrong number of arguments :: Usage :: ./a.out desc_id pos_Training_Dir neg_Training_Dir descFile_save_Name modelFileName\n\n");
		exit(-1);
	}

	//argv[1] contains the descriptor id
	//argv[2] contains the path of the positive images & pos.txt
	//argv[3] contains the path of the negative images & neg.txt
	//argv[4] contains the filename to which descriptors will be written
	//argv[5] contains the model file containing the SVM-parameters
    ...
    histParams[0]= argv[1]; //descriptor id
    histParams[1]=(char*)"8"; //horizontal width of a cell in terms of pixels
    histParams[2]=(char*)"8"; //vertical width of a cell in terms of pixels
    histParams[3]=(char*)"2"; //horizontal width of a block in terms of cells
    histParams[4]=(char*)"2"; //vertical width of a block in terms of cells
    histParams[5]=(char*)orientBinCnt; //no.of orientation bins
    ...
    else
    {
        ...
        //positive images
        string txtFileName = string(argv[2]) + string("pos.txt");
        ...
        //negative images
        txtFileName.clear();
        txtFileName = string(argv[3]) + string("neg.txt");
        ...
        string ymlFileName = string(argv[2]) + string("pos.yml");
...
//***********************************************************************************************************************
        ...
        ymlFileName.clear();
        ymlFileName = string(argv[3]) + string("neg.yml");
        if(access(ymlFileName.c_str(), F_OK) == -1)
        {
